...
**TO DO: AUTOMATISE NUMBER OF SPLITS AT THE END OF THIS CHUNCK:**
...
replacements %<>%
  separate(col = match, 
           into = c("first_replaced", "second_replaced"),
           sep = "/")
...
replacements %>%
  select(replacement, first_replaced, second_replaced)
...
characters.transcripts.df %>%
  mutate(matched.replacement = characters.transcripts.cleaned %>% stringr::str_extract(pattern = rebus::or1(rebus::exactly(replacements$original) )))




%>%
  left_join(replacements, by = c("matched.replacement" = "original"))
...
characters.transcripts.df$characters.transcripts.cleaned %>%
  str_detect(replacements$original) 
