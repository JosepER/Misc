...
characters.transcripts.df %<>%
  mutate(name = name %>% str_replace(pattern = rebus::or("LORD ", "MAESTER ",
                                                         "SER ", " YOUNG ",
                                                         "LADY ", "LADYC "),
                                     replacement = "") )
...
In transcripts, many characters 
```{r eval=TRUE, warning=FALSE, message=FALSE}

#toupper(characters.all) %>% sort %>% utils::View()

characters.all.firstname <- characters.all %>%
  str_match(pattern = capture("[[:upper:]][[:lower:]]+") %R% optional(" .*") %R% " [[:upper:]].+")

characters.all.df <- data_frame(name_1 = characters.all, name_2 = characters.all.firstname[,2])
...
characters.transcripts.df %<>%
  ...
  mutate(name = name %>% str_replace(pattern = "AERSON", replacement = "AERON") %>%
           ...
           str_replace(pattern = or::rebus("PYCELL", "PYRCELLE"), replacement = "PYCELLE") %>%
